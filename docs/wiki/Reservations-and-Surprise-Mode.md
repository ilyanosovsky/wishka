...
- `src/db/access/mutations.ts` and `src/db/access/owner.ts` own the owner-facing **DTO and its raw queries** — every `OwnerWish` read and the bare wish writes (`createWish`/`updateWish`/`deleteWish`/`markGifted`). Both are covered by source-guard tests that assert the module text never mentions the `reservations` table, and `OwnerWish` has no reservation-derived field to begin with — there is nothing to accidentally select. `wish-lifecycle.ts` (below) does not serve owner reads or return an owner DTO; it only *wraps* those writes to add a booking side effect, so the source-guard boundary is unchanged.
...
- Owner-triggered actions that must have a side effect on bookings — delete, edit, mark-gifted — are composed one layer up in `src/db/access/wish-lifecycle.ts`. Each wrapper (`deleteWishAsOwner`, `updateWishAsOwner`, `markGiftedAsOwner`) calls the bare `mutations.ts` write and, in the same transaction, locks the wish row `FOR UPDATE` and captures the id of the booking active at that instant — in the same order whether or not a reservation exists. `mutations.ts`/`owner.ts` remain reservation-blind; the composition that knows about both lives here. The value the owner sees is exactly what the bare mutation returned; the only extra field is `notifyReservationId`, an opaque id destined for `after()` alone (null when no booking is involved, so it cannot be timed or branched on). A test asserts the owner-visible `result` is identical with and without a booking.
- `getReservationNotificationTargetById` (`src/db/access/wish-lifecycle.ts`) resolves that captured id to *who* holds the booking (name, email, locale, guest token). It is **server-side only, consumed exclusively inside `after()`** — the post-response email dispatch in `src/app/wishes/actions.ts`. Its result is never part of a server action's return value, never logged, never rendered. (`getReservationNotificationTarget`, by wish id, serves the guest-confirmation flow in `src/app/reserve/actions.ts`, where the caller just created the booking and scopes the send by guest id.)
...
Every owner trigger site pins its recipient **atomically with the mutation**: `updateWishAsOwner`, `deleteWishAsOwner`, and `markGiftedAsOwner` capture the id of the reservation active at mutation time inside the same transaction, then `after()` resolves it via `getReservationNotificationTargetById(db, reservationId)` and mails the holder. Capturing by id in the transaction is what keeps the email correct under load — the reserver who held the wish at the moment it changed is notified, even if the booking is cancelled or re-taken a millisecond later, before `after()` runs. (The guest-confirmation flow in `src/app/reserve/actions.ts` instead reads the *active* booking on the wish by id and scopes the send by the caller's own guest id, since the guest just created it.)
