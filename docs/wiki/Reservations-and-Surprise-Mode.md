...
- Owner-triggered actions that must have a side effect on bookings — delete, edit, mark-gifted — live in `src/db/access/wish-lifecycle.ts`, not in `mutations.ts`/`owner.ts`. Each wrapper (`deleteWishAsOwner`, `updateWishAsOwner`, `markGiftedAsOwner`) runs the mutation and captures the id of the booking active at that instant **inside the mutation's own transaction**, in the same order whether or not a reservation exists. The value the owner sees is exactly what the bare mutation returned; the only extra field is `notifyReservationId`, an opaque id destined for `after()` alone (null when no booking is involved, so it cannot be timed or branched on). A test asserts the owner-visible `result` is identical with and without a booking.
