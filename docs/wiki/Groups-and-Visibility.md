...
`removeMember` is admin-only, refuses to remove yourself (that's what `leaveGroup`'s succession logic is for), and refuses a target who isn't a member. It never promotes anyone — only a leaving admin triggers succession.
...
- **`revokeGroupInvites(db, groupId, adminId)`** is admin-only and kills every live link of the group at once. There's no per-link UI to revoke a single link, because the only reason to revoke is that a link escaped somewhere, and the group has no way to know how far.
