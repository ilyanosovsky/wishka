...
- For live scraping during development, the `firecrawl` MCP server is registered in the local Claude config (`claude mcp login firecrawl` to authenticate) — that's Path A; no CLI install needed.
