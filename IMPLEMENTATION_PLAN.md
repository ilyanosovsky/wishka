...
- ✅ `updateWishAsOwner` (`src/db/access/wish-lifecycle.ts`) gains an optional `audience` and now also returns `reserverLostAccess: boolean` — computed unconditionally, in the same transaction, by re-running the visibility predicate for the reservation holder captured a moment earlier. The owner-visible `result` is byte-identical whether or not narrowing cut a reserver off.
